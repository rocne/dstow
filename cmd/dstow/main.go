...
import (
	...
	"github.com/rocne/dstow/internal/cli"
)

// version is build metadata, overridden at release time via the goreleaser
// ldflag -X main.version (see .goreleaser/dstow.yaml).
var version = "dev"
...
func main() {
	os.Exit(cli.Run(os.Args, resolveVersion(version, debug.ReadBuildInfo), os.Stdin, os.Stdout, os.Stderr))
}
...
//   - `go install github.com/rocne/dstow/cmd/dstow@v0.1.0` passes nothing, and
