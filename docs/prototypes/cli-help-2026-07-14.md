...
- **Three read surfaces** — `list` enumerates a scope's content; `info`
  reads a scope's fields (config+metadata, no disk); `status` inspects
  reality.
- **`info` introduced** — `info [<name>] [-f <field>]`, brew/apt lineage; the
  §11-sanctioned doorway ("shaping `info` as a named-property read").
  Fields classify as inherent (permanently read-only) vs configured;
  classification is model-level, not command-level. Details → info ticket.
- **No config-mutation commands** — no add/remove/list commands. Declare
...
2. **Info ticket (rescope)**: dependency-query ticket becomes "info: the
   property read" — field vocabulary, inherent/configured
...
5. **CONTEXT.md** (at info-ticket resolution): mint *field*, *inherent
...
§9.2 dependencies — no concept exists to surface (ADR 0002).
